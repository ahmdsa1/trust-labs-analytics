...
def build_report_pdf(patients_df, visits_df, branches_df) -> bytes:
    ...
    try:
        ...
        hp_count = int((patients_df['churn_risk_score'] >= 80).sum()) if 'churn_risk_score' in patients_df.columns else 'N/A'
        gold_count = int((patients_df['patient_tier'] == 'Gold').sum()) if 'patient_tier' in patients_df.columns else 'N/A'

        summary_data = [
            ["Metric", "Value"],
            ["Total Patients", f"{len(patients_df):,}"],
            ["Total Visits", f"{len(visits_df):,}"],
            ["High Risk Patients", str(hp_count)],
            ["Gold Tier Patients", str(gold_count)],
